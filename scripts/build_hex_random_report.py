...
md = md.replace('## 一页结论', '<p class="related"><a href="../battle/index.html">查看局内整体流程与地图种类 →</a> · <a href="../hex-coordinates/index.html">Q/R坐标概念专题 →</a> · <a href="report.md">下载完整 Markdown</a></p>\n\n## 一页结论')
...
'''
page = f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="占城大师地块从初始化、三层随机、隐藏结果保存到扩张显现与点击覆盖的机制复核"><title>占城大师 · 地图与地块生成机制</title><style>{css}</style></head><body><header><div><div class="eyebrow">HEX GENERATION · CODE AUDIT</div><h1>地图与地块生成机制</h1><p>先按一个格子的生命周期理解系统，再查看概率修正、seed、地图池与函数证据。</p><div class="chips"><span>三层随机</span><span>隐藏结果预生成</span><span>点击后覆盖</span><span>387条模板记录</span></div><div class="flow" aria-label="生成流程"><span>地图骨架</span><span>→ 固定/特殊格</span><span>→ 三层随机</span><span>→ 保存隐藏结果</span><span>→ 扩张显现</span><span>→ 点击结算</span></div></div></header><main><nav aria-label="章节目录">{renderer.toc}</nav><article>{body}</article></main><a class="back" href="../../index.html">← 研究首页</a><aside id="tooltip" role="tooltip" hidden></aside><script>{js}</script></body></html>'''
