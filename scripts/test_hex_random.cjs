...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.getByRole('heading',{level:2,name:'先看答案'}).count(),1);
  assert.equal(await page.getByRole('heading',{level:2,name:'2. 开局时每个格子怎样确定'}).count(),1);
  assert.equal(await page.getByRole('heading',{level:2,name:'3. 玩家点击后哪些结果还会改'}).count(),1);
  assert.equal(await page.getByRole('heading',{level:2,name:'5. 地图模板附录'}).count(),1);
  ...
  assert.deepEqual(await page.locator('.template-block').evaluateAll(blocks=>blocks.map(b=>b.open)),[false,false,false,false]);
