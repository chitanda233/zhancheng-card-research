...
(async()=>{
 ...
 for(const arg of process.argv.slice(2).filter(x=>x.startsWith('find='))){
  ...
  const needle=Buffer.from(bytes),direct=!main&&map.has(target)?Buffer.concat([Buffer.from([0x10]),enc(map.get(target))]):null,hits=new Set();bodies.forEach((body,i)=>{if(body.indexOf(needle)>=0||(direct&&body.indexOf(direct)>=0))hits.add(i+imported)});
