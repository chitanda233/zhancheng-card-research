  (func (;47143;) (type 5) (param i32 i32 i32)
    (local i32 i32 i32 i32 i32 i32 i64 i64 i64 i64)
    global.get 0
    i32.const 1344
    i32.sub
    local.set 2
    local.get 2
    global.set 0
    i32.const 11347595
    i32.load8_u
    i32.eqz
    if  ;; label = @1
      i32.const 9821780
      i32.const 1441 ;; 
      call_indirect (type 0)
      i32.const 9821756
      i32.const 1441 ;; 
      call_indirect (type 0)
      i32.const 9899376
      i32.const 1441 ;; 
      call_indirect (type 0)
      i32.const 9890852
      i32.const 1441 ;; 
      call_indirect (type 0)
      i32.const 9886540
      i32.const 1441 ;; 
      call_indirect (type 0)
      i32.const 9890856
      i32.const 1441 ;; 
      call_indirect (type 0)
      i32.const 9827648
      i32.const 1441 ;; 
      call_indirect (type 0)
      i32.const 11347595
      i32.const 1
      i32.store8
    end
    block  ;; label = @1
      block  ;; label = @2
        local.get 1
        i32.load offset=12
        local.set 5
        local.get 0
        local.get 5
        local.get 2
        i32.const 17083 ;; private int FindUnitIndex(int unitId) { }
        call_indirect (type 3)
        local.set 3
        block  ;; label = @3
          local.get 3
          i32.const 0
          i32.lt_s
          br_if 0 (;@3;)
          local.get 2
          i32.const 392
          i32.add
          local.get 0
          i32.load offset=168
          local.get 3
          i32.const 9886540
          i32.load
          i32.const 16747 ;; 
          call_indirect (type 6)
          local.get 2
          i32.load8_u offset=952
          i32.const 1
          i32.and
          i32.eqz
          br_if 0 (;@3;)
          local.get 2
          i32.const 392
          i32.add
          local.get 0
          i32.load offset=168
          local.get 3
          i32.const 9886540
          i32.load
          i32.const 16747 ;; 
          call_indirect (type 6)
          local.get 2
          i32.load offset=396
          local.set 3
          local.get 3
          local.get 1
          i32.load offset=16
          i32.ne
          br_if 0 (;@3;)
          local.get 1
          i64.load offset=40
          local.set 9
          i32.const 9821756
          i32.load
          local.set 4
          local.get 4
          i32.load offset=116
          i32.eqz
          if  ;; label = @4
            local.get 4
            i32.const 339308 ;; 
            call_indirect (type 0)
          end
          local.get 1
          i64.load offset=48
          local.set 10
          i32.const 9821780
          i32.load
          local.set 4
          local.get 4
          i32.load offset=116
          i32.eqz
          if  ;; label = @4
            local.get 4
            i32.const 339308 ;; 
            call_indirect (type 0)
          end
          local.get 1
          i32.load offset=28
          local.set 8
          local.get 1
          i32.load offset=24
          local.set 4
          local.get 1
          i32.load offset=20
          local.set 7
          local.get 2
          local.get 10
          i64.store offset=384
          local.get 2
          local.get 10
          i64.store offset=360
          local.get 2
          local.get 9
          i64.store offset=376
          local.get 2
          local.get 9
          i64.store offset=352
          local.get 2
          i32.const 392
          i32.add
          local.set 6
          local.get 6
          local.get 5
          local.get 3
          local.get 7
          local.get 4
          local.get 2
          i32.const 352
          i32.add
          local.get 8
          i32.const 0
          i32.const 63441 ;; public static BattleSimulation.DamageSource FromDelayedUnitSkill(int unitId, int ownerPlayerId, string unitTypeId, string skillId, FPVector2 position, int attack) { }
          call_indirect (type 23)
          local.get 2
          i32.const 1256
          i32.add
          local.get 6
          i32.const 72
          i32.const 357504 ;; 
          call_indirect (type 3)
          drop
          local.get 2
          i32.const 1328
          i32.add
          local.get 0
          i32.const 0
          local.get 2
          i32.const 17086 ;; private BattleSimulation.ScratchIntListLease RentScratchIntList(int minCapacity = 0) { }
          call_indirect (type 6)
          local.get 2
          i64.load offset=1328
          local.set 11
          local.get 2
          local.get 11
          i64.store offset=1248
          local.get 2
          i32.const 0
          i32.store offset=368
          local.get 2
          local.get 2
          i32.const 1248
          i32.add
          i32.store offset=372
          local.get 1
          i64.load offset=32
          local.set 12
          block  ;; label = @4
            block  ;; label = @5
              block  ;; label = @6
                block  ;; label = @7
                  block  ;; label = @8
                    block  ;; label = @9
                      block  ;; label = @10
                        block  ;; label = @11
                          block  ;; label = @12
                            block  ;; label = @13
                              block  ;; label = @14
                                i32.const 9821756
                                i32.load
                                local.set 4
                                local.get 4
                                i32.load offset=116
                                i32.eqz
                                if  ;; label = @15
                                  i32.const 10787380
                                  i32.const 0
                                  i32.store
                                  i32.const 1443
                                  local.get 4
                                  call 4
                                  i32.const 10787380
                                  i32.load
                                  local.set 4
                                  i32.const 10787380
                                  i32.const 0
                                  i32.store
                                  local.get 4
                                  i32.const 1
                                  i32.eq
                                  br_if 1 (;@14;)
                                end
                                local.get 2
                                i32.const 1336
                                i32.add
                                local.get 10
                                i64.store
                                local.get 2
                                local.get 10
                                i64.store offset=344
                                local.get 2
                                local.get 9
                                i64.store offset=1328
                                i32.const 10787380
                                i32.const 0
                                i32.store
                                local.get 2
                                local.get 9
                                i64.store offset=336
                                local.get 11
                                i64.const 32
                                i64.shr_u
                                i32.wrap_i64
                                local.set 4
                                local.get 0
                                local.get 2
                                i32.const 336
                                i32.add
                                local.get 12
                                local.get 3
                                i32.const 2
                                i32.const 1
                                local.get 5
                                i32.const 0
                                local.get 4
                                local.get 2
                                i32.const 357846 ;; 
                                call_indirect (type 174)
                                i32.const 10787380
                                i32.load
                                local.set 3
                                i32.const 10787380
                                i32.const 0
                                i32.store
                                local.get 3
                                i32.const 1
                                i32.eq
                                br_if 1 (;@13;)
                                local.get 4
                                i32.load offset=12
                                i32.const 0
                                i32.gt_s
                                if  ;; label = @15
                                  i32.const 0
                                  local.set 3
                                  loop  ;; label = @16
                                    i32.const 10787380
                                    i32.const 0
                                    i32.store
                                    i32.const 5777
                                    local.get 4
                                    local.get 3
                                    i32.const 9890856
                                    i32.load
                                    call 6
                                    local.set 7
                                    i32.const 10787380
                                    i32.load
                                    local.set 6
                                    i32.const 10787380
                                    i32.const 0
                                    i32.store
                                    local.get 6
                                    i32.const 1
                                    i32.eq
                                    br_if 4 (;@12;)
                                    i32.const 10787380
                                    i32.const 0
                                    i32.store
                                    i32.const 17083
                                    local.get 0
                                    local.get 7
                                    local.get 2
                                    call 6
                                    local.set 7
                                    i32.const 10787380
                                    i32.load
                                    local.set 6
                                    i32.const 10787380
                                    i32.const 0
                                    i32.store
                                    local.get 6
                                    i32.const 1
                                    i32.eq
                                    br_if 5 (;@11;)
                                    block  ;; label = @17
                                      local.get 7
                                      i32.const 0
                                      i32.lt_s
                                      br_if 0 (;@17;)
                                      local.get 0
                                      i32.load offset=168
                                      local.set 6
                                      i32.const 10787380
                                      i32.const 0
                                      i32.store
                                      i32.const 16747
                                      local.get 2
                                      i32.const 392
                                      i32.add
                                      local.get 6
                                      local.get 7
                                      i32.const 9886540
                                      i32.load
                                      call 13
                                      i32.const 10787380
                                      i32.load
                                      local.set 6
                                      i32.const 10787380
                                      i32.const 0
                                      i32.store
                                      local.get 6
                                      i32.const 1
                                      i32.eq
                                      br_if 8 (;@9;)
                                      local.get 2
                                      i32.load8_u offset=952
                                      i32.const 1
                                      i32.and
                                      i32.eqz
                                      br_if 0 (;@17;)
                                      i32.const 10787380
                                      i32.const 0
                                      i32.store
                                      local.get 2
                                      i32.const 264
                                      i32.add
                                      local.set 6
                                      local.get 6
                                      local.get 2
                                      i32.const 1256
                                      i32.add
                                      i32.const 72
                                      i32.const 357504 ;; 
                                      call_indirect (type 3)
                                      drop
                                      i32.const 17084
                                      local.get 0
                                      local.get 7
                                      local.get 6
                                      local.get 8
                                      local.get 2
                                      call 17
                                      i32.const 10787380
                                      i32.load
                                      local.set 7
                                      i32.const 10787380
                                      i32.const 0
                                      i32.store
                                      local.get 7
                                      i32.const 1
                                      i32.eq
                                      br_if 7 (;@10;)
                                    end
                                    local.get 3
                                    i32.const 1
                                    i32.add
                                    local.set 3
                                    local.get 3
                                    local.get 4
                                    i32.load offset=12
                                    i32.lt_s
                                    br_if 0 (;@16;)
                                  end
                                end
                                local.get 2
                                i32.const 1248
                                i32.add
                                i32.const 0
                                i32.const 17104 ;; public void Dispose() { }
                                call_indirect (type 4)
                                br 7 (;@7;)
                              end
                              i32.const 8684496
                              call 0
                              local.set 3
                              br 5 (;@8;)
                            end
                            i32.const 8684496
                            call 0
                            local.set 3
                            br 4 (;@8;)
                          end
                          i32.const 8684496
                          call 0
                          local.set 3
                          br 3 (;@8;)
                        end
                        i32.const 8684496
                        call 0
                        local.set 3
                        br 2 (;@8;)
                      end
                      i32.const 8684496
                      call 0
                      local.set 3
                      br 1 (;@8;)
                    end
                    i32.const 8684496
                    call 0
                    local.set 3
                  end
                  call 1
                  i32.const 8684496
                  call 9
                  i32.ne
                  br_if 2 (;@5;)
                  local.get 3
                  call 8
                  i32.load
                  local.set 3
                  i32.const 10787380
                  i32.const 0
                  i32.store
                  local.get 2
                  local.get 3
                  i32.store offset=368
                  i32.const 58
                  call 7
                  i32.const 10787380
                  i32.load
                  local.set 4
                  i32.const 10787380
                  i32.const 0
                  i32.store
                  local.get 4
                  i32.const 1
                  i32.eq
                  br_if 1 (;@6;)
                  local.get 2
                  i32.const 1248
                  i32.add
                  i32.const 0
                  i32.const 17104 ;; public void Dispose() { }
                  call_indirect (type 4)
                  local.get 3
                  br_if 6 (;@1;)
                end
                local.get 1
                i32.load offset=92
                i32.const 1
                i32.sub
                local.set 3
                local.get 1
                local.get 3
                i32.store offset=92
                local.get 0
                local.get 5
                local.get 2
                i32.const 17083 ;; private int FindUnitIndex(int unitId) { }
                call_indirect (type 3)
                local.set 5
                local.get 3
                i32.const 0
                i32.le_s
                br_if 3 (;@3;)
                local.get 5
                i32.const 0
                i32.lt_s
                br_if 3 (;@3;)
                local.get 2
                i32.const 392
                i32.add
                local.get 0
                i32.load offset=168
                local.get 5
                i32.const 9886540
                i32.load
                i32.const 16747 ;; 
                call_indirect (type 6)
                local.get 2
                i32.load8_u offset=952
                i32.const 1
                i32.and
                i32.eqz
                br_if 3 (;@3;)
                local.get 0
                i32.load offset=1136
                local.set 5
                local.get 0
                local.get 5
                i32.const 1
                i32.add
                i32.store offset=1136
                local.get 1
                local.get 5
                i32.store
                local.get 1
                i32.load offset=96
                local.set 5
                local.get 0
                i32.load offset=1244
                local.set 4
                i32.const 9827648
                i32.load
                local.set 7
                local.get 7
                i32.load offset=116
                i32.eqz
                if  ;; label = @7
                  local.get 7
                  i32.const 339308 ;; 
                  call_indirect (type 0)
                end
                local.get 1
                local.get 3
                i32.const 1
                i32.eq
                i32.store8 offset=64
                local.get 5
                i32.const 1
                local.get 5
                i32.const 1
                i32.gt_s
                select
                local.get 4
                i32.add
                local.set 3
                local.get 1
                local.get 3
                i32.store offset=8
                local.get 1
                local.get 3
                i32.store offset=4
                local.get 2
                i32.const 136
                i32.add
                local.set 3
                local.get 3
                local.get 1
                i32.const 128
                i32.const 357504 ;; 
                call_indirect (type 3)
                drop
                local.get 0
                local.get 3
                local.get 2
                call 23993
                i32.const 9899376
                i32.load
                local.set 3
                local.get 0
                i32.load offset=256
                local.set 0
                local.get 2
                i32.const 392
                i32.add
                local.get 1
                i32.const 128
                i32.const 357504 ;; 
                call_indirect (type 3)
                drop
                local.get 0
                local.get 0
                i32.load offset=16
                i32.const 1
                i32.add
                i32.store offset=16
                local.get 0
                i32.load offset=12
                local.set 1
                local.get 0
                i32.load offset=8
                local.set 5
                local.get 1
                local.get 5
                i32.load offset=12
                i32.ge_u
                br_if 2 (;@4;)
                local.get 0
                local.get 1
                i32.const 1
                i32.add
                i32.store offset=12
                local.get 5
                local.get 1
                i32.const 7
                i32.shl
                i32.add
                i32.const 16
                i32.add
                local.get 2
                i32.const 392
                i32.add
                i32.const 128
                i32.const 357504 ;; 
                call_indirect (type 3)
                drop
                br 3 (;@3;)
              end
              call 10
              local.set 3
              call 1
              drop
            end
            i32.const 10787380
            i32.const 0
            i32.store
            i32.const 17085
            local.get 2
            i32.const 368
            i32.add
            call 2
            drop
            i32.const 10787380
            i32.load
            local.set 0
            i32.const 10787380
            i32.const 0
            i32.store
            local.get 0
            i32.const 1
            i32.eq
            br_if 2 (;@2;)
            local.get 3
            call 11
            unreachable
          end
          local.get 3
          i32.load offset=16
          i32.load offset=96
          i32.load offset=56
          local.set 1
          local.get 2
          i32.const 8
          i32.add
          local.set 3
          local.get 3
          local.get 2
          i32.const 392
          i32.add
          i32.const 128
          i32.const 357504 ;; 
          call_indirect (type 3)
          drop
          local.get 0
          local.get 3
          local.get 1
          i32.const 4078 ;; 
          call_indirect (type 5)
        end
        local.get 2
        i32.const 1344
        i32.add
        global.set 0
        return
      end
      i32.const 0
      call 0
      drop
      call 1
      drop
      call 998
      unreachable
    end
    local.get 3
    i32.const 5634 ;; 
    call_indirect (type 0)
    unreachable)