...
4. 用户标签金卡加成为：刚付费+10、连败+50、回流+30、连胜+50个百分点，可叠加。首次生成符合条件的SSSR兵营类结果后，该玩家的这条标签加成会被标记为已重置。
